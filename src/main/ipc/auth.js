...
export function registerAuthIpc(ipcMain, store) {
    ipcMain.handle('wou:get-session', () => {
        return {
            token: store.get('wou_session_token'),
            user: store.get('wou_user_data')
        };
    });
    ipcMain.handle('wou:set-session', (_e, { token, account }) => {
    ...
    ipcMain.handle('wou:proxy', async (_e, { path, method = 'GET', body, headers }) => {
        const token = store.get('wou_session_token');
    ...
    ipcMain.handle('wou:open-external', async (_e, url) => {
        const { shell } = await import('electron');
        await shell.openExternal(url);
        return { ok: true };
    });
    // OAuth: system browser + loopback http://127.0.0.1:0/callback
    ipcMain.handle('wou:oauth', async (_e, provider) => {
        const { shell } = await import('electron');
        ...
        return new Promise((resolve, reject) => {
            const server = http.createServer(async (req, res) => {
                try {
                    ...
                    let account;
                    try {
                        account = JSON.parse(decodeURIComponent(accountRaw));
                    }
                    catch {
                    ...
                    setTimeout(() => {
                        try {
                            server.close();
                            console.log('[Hyper][main][wou:oauth] loopback server closed, resolve');
                        }
                        catch { }
            ...
            server.listen(0, '127.0.0.1', () => {
                ...
                const accountId = store.get('wou_user_data')?.id ?? '';
